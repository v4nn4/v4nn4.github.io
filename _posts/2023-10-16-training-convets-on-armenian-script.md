...
Each image has $$N^2 = 56^2 = 3136$$ values in range $$[0, 255]$$, or rather $$[0, 1]$$ after normalization. We denote by $$\textbf{x}_i \in \mathbb{R}^{N^2}$$ the pixel intensities of the i-th image. Our goal is to find a mapping between those pixel intensities, and our target labels. This mapping should be representing by a function  $$f : \mathbb{R}^{N^2} \rightarrow \mathbb{R}^{C}$$ where $$C = 76$$ is the number of classes of our classification problem. For each image $$\textbf{x}$$, $$\textbf{y}=f(\textbf{x})$$ will represent the probability that the image belongs to each class.

We will use the the LeNet-5 model, a Convolutional Neural Network (CNN) introduced by Yann LeCun in 1998[^1]. This model has been successfully used on handwritten digits recognition, where the number of classes is 10 (see MNIST[^2] dataset). Our intuition is that the model captures how character strokes interact with each other thanks to its convolution kernels. Our dataset have more classes, higher resolution, as well as characters with more complicated shapes, like `թ` or `ֆ`. But essentially, the problem is similar, and we believe convolutional kernels should adapt to our dataset without too much trouble.
